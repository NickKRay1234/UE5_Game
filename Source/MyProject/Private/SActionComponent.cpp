...
bool USActionComponent::StartActionByName(AActor* Instigator, FName ActionName) // Function to start an action based on its name.
{
	for(USAction* Action : Actions) // Iterate through all actions in the component's list of actions.
	{
		if(Action && Action->ActionName == ActionName) // Check if the current action is valid and matches the provided action name.
		{
...
bool USActionComponent::StopActionByName(AActor* Instigator, FName ActionName) // Function to stop an action based on its name.
{
	for(USAction* Action : Actions) // Iterate through all actions in the component's list of actions.
	{
		if(Action && Action->ActionName == ActionName) // Check if the current action is valid and matches the provided action name.
...
void USActionComponent::AddAction(TSubclassOf<USAction> ActionClass)
{
	...
	if(USAction* NewAction = NewObject<USAction>(this, ActionClass)) // Create a new action object of the provided class.
